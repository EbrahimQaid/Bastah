...
CREATE TABLE IF NOT EXISTS products (
  ...
  -- Variants stored in separate table (see below)
...
-- Full-text search index (Arabic support via pg_trgm)
...
CREATE TABLE IF NOT EXISTS orders (
  ...
  -- Customer Snapshot (في حال تغيير بيانات العميل لاحقاً)
  ...
  payment_method      TEXT DEFAULT 'cod'
                      CHECK (payment_method IN ('cod','stripe','moyasar','bank_transfer','other')),
  ...
  source              TEXT DEFAULT 'store'
                      CHECK (source IN ('store','whatsapp','phone','other')),
  -- Coupon
  coupon_code         TEXT,
  coupon_id           UUID,
  -- Timestamps
...
-- ============================================================
-- 13. STORE ANALYTICS — تحليلات المتجر (مجمّعة يومياً)
-- ============================================================
...
CREATE TABLE IF NOT EXISTS notifications (
  ...
  type        TEXT NOT NULL,                          -- 'new_order','low_stock','review', etc.
  title       TEXT NOT NULL,
  body        TEXT,
  data        JSONB DEFAULT '{}',
...
CREATE TABLE IF NOT EXISTS audit_logs (
  ...
  entity      TEXT NOT NULL,                          -- 'product','order','category', etc.
  entity_id   UUID,
  old_data    JSONB,
  new_data    JSONB,
  ip_address  TEXT,
...
CREATE INDEX idx_audit_store_id  ON audit_logs(store_id, created_at DESC);
...
-- Plans
INSERT INTO plans (name, name_ar, price_monthly, price_yearly, max_products, max_orders, max_images, features)
VALUES
  ('free',       'مجاني',      0,    0,    20,   100,  3,  '["متجر إلكتروني","واتساب"]'),
  ('pro',        'احترافي',    99,   990,  200,  1000, 10, '["كل ميزات المجاني","كوبونات","تحليلات","دعم أولوية"]'),
  ('enterprise', 'مؤسسي',      299,  2990, 99999,99999,20, '["كل ميزات الاحترافي","API كامل","مدير حساب","SLA"]')
