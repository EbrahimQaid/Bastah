...
VALUES (1, 1, 1, 'dukkani', 'دكاني - Dukkani', 'متجر دكاني', 'دكانك الرقمي بين يديك', '#991B1B', '#DC2626', 'Tajawal', '967770000000', 'YER', ARRAY['YER', 'SAR', 'USD'])
ON CONFLICT (id) DO NOTHING;
...
-- ============================================================
-- PRODUCTION HARDENING — safe to run after the base schema
-- ============================================================

-- Tenant-scoped slugs prevent ambiguous URLs and improve lookups.
CREATE UNIQUE INDEX IF NOT EXISTS idx_categories_store_slug
  ON categories(store_id, slug) WHERE slug <> '';
CREATE UNIQUE INDEX IF NOT EXISTS idx_products_store_slug
  ON products(store_id, slug) WHERE slug <> '';

-- Reject invalid commerce values at the database boundary.
DO $$
BEGIN
...
CREATE INDEX IF NOT EXISTS idx_order_items_store_order
